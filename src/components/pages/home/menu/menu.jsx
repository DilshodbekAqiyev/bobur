    ...
    <div className={`${styles}`}>
      <div className={`bg-grey dark:bg-slate-800 p-[24px] `}>
        ...
        <ul className="flex flex-col ml-[12px] mt-[10px]">
          {plantTypes.map((type,i)=>i<9&&<li className="flex items-center justify-between text-2 text-black  dark:text-white hover:text-green hover:font-bold transition leading-2 font-normal py-[10px]">
            <p>{type.type}</p>
            <p>({type.count})</p>
          </li>)}
        </ul>
        ...
        <ul className="ml-[12px]">
          {plantTypes.map((type,i)=>i>8&&<li className="flex items-center justify-between text-2 text-black dark:text-white hover:text-green hover:font-bold transition leading-2 font-normal py-[10px]">
            <p>{type.type}</p>
            <p>({type.count})</p>
          </li>)}
        </ul>
